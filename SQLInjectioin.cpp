...
{
    // Clear any prior query results before running a new query.
    records.clear();

    /*
      Detect the specific SQL injection pattern required by the assignment:

        OR value=value;

      The value may be:
        - a number: 1=1, 2=2, -5=-5, 3.14=3.14
        - a quoted string: 'hi'='hi', 'hack'='hack'

      This avoids blocking normal SQL keywords like WHERE, FROM, PASSWORD, AND, etc.
      We are not using a broad blacklist. We are only detecting the specific
      always-true OR literal = same literal pattern.
    */
    std::regex injection_pattern(
        R"SQL(\bor\s+((?:'[^']*')|(?:-?\d+(?:\.\d+)?))\s*=\s*\1\s*;?\s*$)SQL",
        std::regex_constants::icase
    );

    if (std::regex_search(sql, injection_pattern))
    {
        std::cout << "Potential SQL Injection detected and blocked: " << sql << std::endl;
        return false;
    }
